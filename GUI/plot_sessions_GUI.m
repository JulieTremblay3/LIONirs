...
%load zone
try %to be implement save in the NIRS.mat
    set(handles.popupmenu_zone,'string',{''});
    set(handles.popupmenu_zone,'value',1);
   [filepath,name,ext] = fileparts(handles.NIRS.Dt.fir.pp(1).p{1}); 
    load([filepath,filesep, 'myzone.zone'],'-mat'); 
        PMI{1}.zone.plot = zone.plot;
        PMI{1}.zone.plotLst = zone.plotLst;
        PMI{1}.zone.label = zone.label;
         PMI{1}.zone.color =  zone.color;
        set(handles.popupmenu_zone,'string', PMI{1}.zone.label);
        set(handles.popupmenu_zone,'value',numel( PMI{1}.zone.label));      
    end
catch
    set(handles.popupmenu_zone,'string',{''});
    set(handles.popupmenu_zone,'value',1);
...
if 1 %numel(PMI{currentsub}.plotLst)==1
    ...
    rall(find(MeasListAct==0)) = 0; %set to zero rejected channel 
 ...
 PMI{currentsub}.plotLst 
disp(['Select channels in the visualisation with a correlation higher than ',num2str(corrcoef), ' of the current selected channel averaged', ...
    'this option is useful to apply decomposition on an easily identified artifact and find channels with a similar event. ',...
    'Separately it also creates a zone grouping similar signature based on their correlation.' ...
    'Rejected channels are ignored for this correlation'] )
...
if type == 1
    ...
    [filepath,name,ext] = fileparts(handles.NIRS.Dt.fir.pp(1).p{1})
    [name, path]= uiputfile('.zone','Save the zone definition', [filepath,filesep, 'myzone.zone'] );
    zone = DOT{currentsub}.zone;
    if name == 0
        return
    end
    ...
    [filepath,name,ext] = fileparts(handles.NIRS.Dt.fir.pp(1).p{1})
    [name, path]= uigetfile('.zone','Open the zone definition', [filepath,filesep, 'myzone.zone'] ) ;
    try
    load([path, name],'-mat');
    catch
       return
...
indstart= find(posxstart>PMI{currentsub}.data(cf).HRF.tHRF);
    ...
    disp('Please select an interval')
    return
end
indstop= find(posxstop>PMI{currentsub}.data(cf).HRF.tHRF);
...
if strcmp(listmethod{idval},'Parafac')  %substractPARAFAC
    ...
    PMI{1}.tmpPARAFAC.dotted(ComponentToKeep) = 1;
    ...
    set(guiHOMER,'UserData',PMI);
    gui_ParafacIO() %,gcbo,[],guidata(gcbo))
...
if strcmp(ext,'.dat')| strcmp(ext,'.eeg')| strcmp(ext,'.nir')%Generic Data export analyser
    ...
    if isfield(PMI{1},'EEGoption') %NIRS.Cf.H.C.ok(Idx,f) ~= 0;  %If the channel was not previously flagged. %DO ALL
            ...
            if PMI{1}.EEGoption.filter
                for Idx=1:size(dall,2)
                   dinterp =  dall;
                                ...
                                lowcut = PMI{1}.EEGoption.filterLPF;
                                highcut =  PMI{1}.EEGoption.filterHPF;
                                if 1
                                    Wn = lowcut*2/fs;
...
if strcmp(listmethod{idval},'Parafac') %get(handles.popupmethodselected,'value')==1  %extract PARAFAC
    ...
    %   PMI{currentsub}.data(cf).HRF.AvgC(indt,:) = reshape(data,[numel(indt),size(d,2)]);
        ...
        msgbox('Press the button ''Get'' before the component could be add to the list');
        return
    end
    
elseif strcmp(listmethod{idval},'PCA')  %extract PCA get(handles.radio_PCA,'value') %substract
     if isfield (PMI{currentsub}, 'tmpPCA') % to make sure the user press GET
        %Detrent DATA segment for centrering
        indt = [PMI{currentsub}.tmpPCA.indt(1):PMI{currentsub}.tmpPCA.indt(2)];%Time indice
        intensnorm = d(indt,:);
        X = 1:1:size(intensnorm,1);
...
try
    ...
    switch ME.identifier
        case 'MATLAB:UndefinedFunction'
            ...
            rethrow(ME)
            disp(['Maybe Add NPLS fonction in matlab path: addpath(''',pathname(1:end-4), '\NPLS'')'])
        otherwise
            rethrow(ME)
    end
end   
...
elseif strcmp(get(handles.HbO_Context_Select_Bad,'Checked'),'on')
    ...
    MeasListAct(:)=1;
    disp('Select HbO channel, including  rejected channel')
end
plotLst= find(MeasListAct& PMI{currentsub}.data(cf).MeasList(:,4)==1);
if ~isempty(plotLst)
    PMI{currentsub}.plotLst = plotLst;
    set(handles.edit_plotLst,'string',mat2str(plotLst))
    PMI{currentsub}.plot = [PMI{currentsub}.data(cf).MeasList(plotLst,1),PMI{currentsub}.data(cf).MeasList(plotLst,2)];
...
elseif strcmp(get(handles.HbO_Context_Select_Bad,'Checked'),'on')
    ...
    MeasListAct(:)=1;
    disp('Select HbR channel, including rejected channel')
end


plotLst= find( MeasListAct & PMI{currentsub}.data(cf).MeasList(:,4)==2);
if ~isempty(plotLst)
    PMI{currentsub}.plotLst = plotLst;
    set(handles.edit_plotLst,'string',mat2str(plotLst))
    PMI{currentsub}.plot = [PMI{currentsub}.data(cf).MeasList(plotLst,1),PMI{currentsub}.data(cf).MeasList(plotLst,2)];
...
function HbO_Context_Select_Good_Callback(hObject, eventdata, handles)
    if strcmp(get(handles.HbO_Context_Select_Good,'Checked'),'on')
        ...
        set(handles.HbO_Context_Select_Good,'Checked',1)
...
if strcmp(get(handles.Context_MenuExportZone_All,'checked'),'off') %only one curve
    ...
    if isfield(PMI{currentsub}.data(cf).HRF,'AvgStdErr');
        for ichHBO = 1:numel(chHbO)
            ...
            set(he,'Displayname',['HbO',label,'ch',num2str(chHbO( ichHBO)),'_',srs, '_', det]);      
        end
...
tstart = str2num(get(handles.edit_time_start,'string'));
tstop = str2num(get(handles.edit_time_stop,'string'));
if num2str(get(handles.edit_start,'string'))==(tstart-1.5)
    set(handles.edit_start,'string', num2str(tstart-0.75));
    set(handles.edit_duration,'string', num2str(tstop-tstart+1.5));
elseif num2str(get(handles.edit_start,'string'))==(tstart-3)
    set(handles.edit_start,'string', num2str(tstart-1.5));
    set(handles.edit_duration,'string', num2str(tstop-tstart+3));
else
    set(handles.edit_start,'string', num2str(tstart-3));
    set(handles.edit_duration,'string', num2str(tstop-tstart+6));
end
...
tstart = str2num(get(handles.edit_start,'string'));
duration = str2num(get(handles.edit_duration,'string'));
set(handles.edit_start,'string', num2str(tstart-1));
set(handles.edit_duration,'string', num2str(duration+2));
guiHOMER = getappdata(0,'gui_SPMnirsHSJ');
currentsub=1;
...
namefile = get(handles.popupmenu_file,'string');
valuefile = get(handles.popupmenu_file,'value');
...
%for f=1:size(rDtp,1)
 %   d = fopen_NIR(rDtp{f,1},NC);           
    samp_length = size(PMI{currentsub}.data(1).HRF.noise,1);
    ...
    imagesc(time,1:size(PMI{currentsub}.data(1).HRF.noise,1),PMI{currentsub}.data(1).HRF.noise');
    ylabel('Channel id number','fontsize', 8);
    title([namefile{valuefile}],'fontsize', 10);
    set(gca,'fontsize',8);
...
% --------------------------------------------------------------------
function menu_settings_EEG_Callback(hObject, eventdata, handles)
% hObject    handle to menu_settings_EEG (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function menu_context_trigger_allon_Callback(hObject, eventdata, handles)
% hObject    handle to menu_context_trigger_allon (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

triggerinfo = get(handles.popupmenu8,'string');
%itriggerinfo = get(handles.popupmenu8,'value');
for itriggerinfo=1:numel(triggerinfo)
a = triggerinfo{itriggerinfo};
triggerinfo{itriggerinfo} = [a(1:end-4),'  on'];
end
set(handles.popupmenu8,'string', triggerinfo)
...
% --------------------------------------------------------------------
function menu_context_trigger_alloff_Callback(hObject, eventdata, handles)
% hObject    handle to menu_context_trigger_alloff (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function context_AVGwithSTDerror_Callback(hObject, eventdata, handles)
% hObject    handle to context_AVGwithSTDerror (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if strcmp(get(handles.context_AVGwithSTDerror,'checked'),'on');
    set(handles.context_AVGwithSTDerror,'checked','off');
else
