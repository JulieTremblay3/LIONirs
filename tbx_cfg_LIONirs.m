...
E_aux2manualoption.help    = {'Separate file : identify each trig as a separe file.',...
    ...
    'this option will ajust the trig time assuming that each file from the project are place,'...
...
E_writeNIRS.val  = {NIRSsession,NIRSmat,NIRS_exportoption,NIRSname};
...
v_component_list_remove.help    = {['Allow to remove few component that could be outlier for mean and avg topography, set zeros keep all, else write down the index to remove ']}; 
...
E_exportcomponent_list.help = {'Export component spatial contribution to perform group average or statistic'};
...
e_zonetoextract_label.help    = {'Identify the label to identify the zones to export, separate different label using a comma as example zone1 , zone2, zone3'};
...
c_zonetoextract.help    = {'List label of zone directly or through a .txt file'};
...
E_exportcomponent_zone.help = {'Export component spatial contribution to perform group average or statistic'};
...
m_TtestOneSample.help = {'Use 2 tail or on tail treshold'};
...
f_component.help    = {'Enter the list of component to test statisticly'}; 
...
f_componentG1.name    = 'Groupe 1 component'; 
...
f_componentG2.name    = 'Groupe 2 component'; 
...
f_componentG2.help    = {'Enter the list of component to test statisticly. D1matrix or mean by subject from the export list.'}; 
...
i_videooffset_yes.help    = {'As example if the in video the event occur at 5 seconde and the trig in EEG happen at 10 you should ajust the lag to -5 as AUDIO(5)- EEG(10)=LAG(-5)'};;
...
i_Audiooffset.val     = {0}; 
i_Audiooffset.help    = {'As example if the in audio the event occur at 5 seconde and the trig in EEG happen at 10 you should ajust the lag to -5 as AUDIO(5)- EEG(10)=LAG(-5)'};
...
i_ch_crossspectrum.help    = {'Plot and save the spectrum in for zone, it will be save in the same folder than the matrix'}; 


%Input Frequency
i_TrialLenght_crossspectrum          = cfg_entry; 
i_TrialLenght_crossspectrum.name    = 'Trial lenght (s)'; 
...
i_TrialLenght_crossspectrum.help    = {'Lenght of the segments ajust to change FFT frequency measured'}; 
...
i_RandomSample_crossspectrum.help    = {'Number of random sample to compute the cross spectrum, they are take using pseudorandom segment in the whole bloc'}; 
...
m_savefft_crossspectrum.help     = {'Save FFT spectrum'};
...
E_GUI_lookmatrices.help = {'Open a gui to visualised fNIRS connectivity matrix'};

m_TtestOneSample_matrix      = cfg_menu;
m_TtestOneSample_matrix.tag  = 'm_TtestOneSample_matrix';
m_TtestOneSample_matrix.name = 'Use ';
m_TtestOneSample_matrix.labels = {'CH 1 tail right (pos) ' };
m_TtestOneSample_matrix.values = {1,2,};
m_TtestOneSample_matrix.val  = {1}; 
m_TtestOneSample_matrix.help = {'Test option'};
...
b_TtestOneSamplematrix.val    = {f_matrix,m_TtestOneSample_matrix};
b_TtestOneSamplematrix.help   = {'The one-sample t-test is a parametric test of the location parameter when the population standard deviation is unknown. T = mean(X)/(STD(x)*sqrt(n))'};


c_statmatrix          = cfg_choice;
c_statmatrix.tag     = 'c_statmatrix';
c_statmatrix.name    = 'Choose the statistical test';
c_statmatrix.values  = {b_TtestOneSamplematrix};
