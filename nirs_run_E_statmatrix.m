...
elseif isfield(job.c_statmatrix,'b_GLM_Mat')
    ...
    %Crée des matrices de zéros ayant la même taille que MATall (65x65)
    for icov = 1:numel(covariableall)
        eval(['bCOV',num2str(icov),' = zeros(size(MATall,2),size(MATall,2));']);
        eval(['bCOV',num2str(icov),'sig = zeros(size(MATall,2),size(MATall,2));']);
    end
    
    for i=1:size(MATall,2)%pour chaque canal i (possibilité de valeurs de 0 à MATall donc 65)
        for j=1:size(MATall,2)%pour chaque canal j (possibilité de valeurs de 0 à MATall donc 65)
