function out = nirs_run_readBoxyISS(job)
...
%         NIRS.VMRKfile{Idx_file} = fileOutRoot_vmrk;
        ...
        nirs_boxy_write_markers(fileOutRoot_vmrk,... %Output file
            ...
            try
                ...
                if isempty(SD.aux5)%Force un trig 255 au début de l'enregistrement pour tj avoir un trig même si enregistrement n'en contient pas. 
                    desc = 255;
                    aux_ind = 1;
                end
                if 1
                ...
                SD.auxtrig{Idx_file} = [new_desc, aux_ind2];
    ...
    NIRS.Dt.fir.pp(1).sizebloc{Idx_file,1} = SD.sizebloc;
    NIRS.Dt.fir.pp(1).p{Idx_file,1}=fileOut;
