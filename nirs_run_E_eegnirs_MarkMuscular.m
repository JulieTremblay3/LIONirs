function out = nirs_run_E_eegnirs_MarkMuscular(job)
%Remode linear trend
%filename prefix
prefix = 'o'; %for "onset"
...
for filenb=1:size(job.NIRSmat,1) %Loop over all subjects
    ...
    for f=1:size(rDtp,1) %Loop over all files of a NIRS.mat
        ...
        if 1 %EEGdirect
           filelist = rDtp{f,1};
           [dataseg,info,label,ind_dur_ch] = fopen_EEG(filelist );
        ...
        %load eeg
        offset = NIRS.Dt.EEG.pp(end).sync_timesec{filenb};
        filelist = NIRS.Dt.EEG.pp(end).p{filenb};
        [dirNPLS,file,ext] = fileparts(filelist);
        tstartEEG = offset;
        tstopEEG = size(d,2)*1/fs + offset;
        [dataseg,info,label,ind_dur_ch] = fopen_EEG(filelist, tstartEEG,tstopEEG);

        [dataseg,info,label,ind_dur_ch] = fopen_EEG(filelist, tstartEEG, tstopEEG );
        end
        % Downsample to 256 to perform wavelet transform
        FsEEG = 1/(info.SamplingInterval*1e-6);  %Hz?
        tic
        freqVec = [20,25,30,35,40,45,50]; % Hz
        width = 7 ;
        tic; TFRseg = morletcwtEd(dataseg(:,:),freqVec,FsEEG,width); tori=toc; % Complex Wavelets Coefficients
        S =  abs(TFRseg);
        toc
        tEEG = 1/FsEEG:1/FsEEG:1/FsEEG*size(TFRseg,1);

        % figure
        % hold on
        % %for iele=1:numel(info.name_ele)
        % plot(tEEG, sum(sum(S (:,:,:),3),2)) %, 'displayname', info.name_ele{iele});
        % xlabel(['tNIRS offset eeg' num2str( offset)])
        % %end



        sumEEGGamma = sum(sum(S (:,:,:),3),2);
        LowPass_filter =2;
        Wn = LowPass_filter*2/FsEEG;
        filt_ord =4;
        [fb,fa]=butter(filt_ord,Wn,'low');
        idx = 1;
        dfilt = filtfilt(fb,fa, sumEEGGamma);
        disp(['Apply ',  num2str(LowPass_filter),' Hz lowpass filter for the detection.'])

        
       
        %set detection threshold using zscore but excluding all outlier
         %value
        dfiltclean = dfilt;
        dfiltclean(abs(zscore(dfilt(:)))>1.5)=nan;
        moyenne = nanmean( dfiltclean(:));
        standard = nanstd( dfiltclean(:));
        zwithououtlier = (dfilt-moyenne)./standard ;
        seuil = 1.287;
        muscular = ( zwithououtlier > seuil );

stepmat = muscular'
min_sub =   sum((tEEG<2)); %2 secondes 
if 1 %m_min_subinterval
            for Idx = 1:1
                step_dur = 0;
                count_sub_dur = false;
                sub_dur = 1;
...
% 
% figure;imagesc(stepmat)
% figure;imagesc(muscular')
...
muscular = stepmat;
        ...
        try
            idoffset = sum((tEEG<offset))
        catch
            offset=0;
            idoffset  = 0;
        end
        figure;hold on
        plot(tEEG,sumEEGGamma,'displayname','sum gamma')
        plot(tEEG,dfilt,'displayname','sum gamma with low pass filter')
        xlabel(['tNIRS offset eeg' num2str( offset)])
        plot(tEEG, muscular*moyenne,'displayname','Musculardectected')
        noise=zeros(info.DataPoints,1);
        outfilevmrk = [filelist(1:end-4),'.vmrk'] 
        noise(find(muscular)+idoffset,:)=1;
        [ind_dur_ch] = mat2d2ind_dur_ch(noise');
        ind_dur_ch(:,3) = 0; %apply to all channels 
        write_vmrk(outfilevmrk,'bad_step','muscular',ind_dur_ch);
        disp(['Write bad step muscular in ', outfilevmrk])
    end



    % 
    % tNIRS= 1/fs:1/fs:size(d,2)*1/fs;
    % 
    % iNIRSart = find(tNIRS>55.538 & tNIRS<58 )
    % figure
    % plot( tNIRS(iNIRSart), d(:, iNIRSart)')
