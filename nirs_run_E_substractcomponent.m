    ...
    for icomporder=1:numel(PARCOMP) %Loop over all files of a NIRS.mat
        ...
        if ~isempty(strfind(PARCOMP(icomp).label, type))%si bon label
            ...
            if job.m_substract_and_offsetcorrection==1   
                ...
                if indstart == 1 
                d(PARCOMP(icomp).indt(1):PARCOMP(icomp).indt(end) ,listgood2wv) = spar + ones(numel(indt),1)*d(indstart,listgood) ;
                 d(indstop+1:end,listgood) =  d(indstop+1:end,listgood)-...
                    ones(size(d(indstop+1:end,1)))* (d(indstop+1,listgood) - d(indstart,listgood) );
                else
