...
if isfield(NIRS.Dt,'AUX')
    ...
    NIRS.Dt.AUX.pp.p{size(rDtp,1),1}=[];
    NIRS.Dt.AUX.pp.sync_timesec{size(rDtp,1),1}=[];
    NIRS.Dt.AUX.label = [];
end
for ifile =1:numel(rDtp)
   ...
   id=  find(sum(aux5(:,1)==trigtofind,2));
   if ~isempty( id)
    onset =aux5(id,2).*1./fs;
    dur = repmat(job.c_createAUXauto.b_HRFtriggeronset.e_HRFduration,numel(onset),1) ;
    weight =repmat(1,numel(onset),1) ;
   end
