  ...
  for ind = 1:length(PMI{currentsub}.plotLst);
    ...
    if (srs ~= 0 && det ~= 0 )     
      ...
      if flag_display
        ...
        if DispParameter.dist
            x = ((srsx+detx)/2)+0.005;
            y = (srsy+dety)/2+0.005;
            z = (srsz+detz)/2+0.005;
            text(x,y,z,d_str,'color','black','FontSize',14);
        end
        if DispParameter.idnb
            x = ((srsx+detx)/2)+0.005;
            y = (srsy+dety)/2+0.005;
            z = (srsz+detz)/2+0.005;
      ...
      if flag_display
        ...
        if DispParameter.idnb               
            if srsy2>0
                ...
                y = (srsy2+dety2)/2+0.005;
                z = (srsz2+detz2)/2+0.005;
                text(x,y,z,num2str(PMI{currentsub}.plotLst(ind)),'color','black','FontSize',14,'BackgroundColor',[1,1,1]);
            else
                x = ((srsx2+detx2)/2)-0.005;
                y = ((srsy2+dety2)/2)-0.005;
                z = ((srsz2+detz2)/2)-0.005;
        ...
        if DispParameter.nbavg
            if isfield(PMI{currentsub}.data(cf).HRF, 'navg') % Verify that the field exists to avoid accessing a non-existent field.
                x = ((srsx2+detx2)/2)+0.005;
                y = (srsy2+dety2)/2+0.005;
                z = (srsz2+detz2)/2+0.005;
