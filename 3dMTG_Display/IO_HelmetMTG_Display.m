...
if isfield(option,'SPMgui')
     ...
     set(handles.popupmenu_display,'string',{'Current module','Topo','Mean start stop time','Projection Channel .cp','PARAFAC','ICA','PCA','Detrend','Current module baseline correction at start time','PARAFAC WAVELET','GLM','COMPONENT'});
end
...
elseif get(handles.radio_guiSPMnirsHSJ,'value')==2
    ...
    if type==1 %Current module value
...
%           figure;plot(temp)
    ...
    elseif type==9 %Current module set tstart to zero.
      ...
      elseif  type==10 %PARAFAC wavelet
    ...
    elseif type==12 %Selected component topo
          d1 = zeros( size(PMI{1}.data.MeasListAct,1)/2,1); 
         ...
         if dconc==1
            d1(listgoodHbO,1) =topoHbO;
         elseif dconc==2
             d1(listgoodHbO,1)=topoHbR;
         end
...
DispParameter.idnb = get(handles.radio_IDnb,'value');
DispParameter.nbavg = get(handles.radio_nbavg,'value');
