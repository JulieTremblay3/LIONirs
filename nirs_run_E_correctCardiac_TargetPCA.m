function out = nirs_run_E_correctCardiac_TargetPCA(job)
%Remode linear trend
%filename prefix 
...
%             try
                ...
                sizebloctocheck = size(d',1);     
                 cardiac_trigger = str2num(job.e_onset_trig);

                 tmpaux5 = NIRS.Dt.fir.aux5{1,1}; 
                idmarkercardiac = find( tmpaux5(:,1)==cardiac_trigger);
               maxpeak = tmpaux5( idmarkercardiac,2) ;  
