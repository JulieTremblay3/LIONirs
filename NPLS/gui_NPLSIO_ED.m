...
%if 1 ; dataseg = [fliplr(dataseg);dataseg;fliplr(dataseg)]; end%******???
...
Fs = 1/(info.SamplingInterval*1e-6);  %Hz? 
%t_eeg = (0 : size(dataseg,1)-1)./Fs;

% Downsample to 256 to perform wavelet transform 
Fsnew = 256;
dataseg1 =  resample_old( dataseg, Fsnew, Fs);
dataseg = dataseg1; clear dataseg1 % this is just security reasons to check here if resampling is going well
Fs = Fsnew;
%tEEG = (0 : size(dataseg,1)-1)./Fs;
%figure;plot(tEEG,dataseg(:,:))
freqVec = 1:1:40; % Hz
width = 7 ;
tic; TFRseg = morletcwtEd(dataseg,freqVec,Fs,width); tori=toc; % Complex Wavelets Coefficients
S =  abs(TFRseg);   %non-normalized
%S =  abs(TFRseg)./mean(mean(abs(TFRseg),3),1).*ones(size(TFRseg));%normalized
%figure;imagesc(tEEG,freqVec,S(:,:,1)');axis xy
...
if isfield( PMI{currentsub}.data(cf),'AUX')  
 ...
 %% Downsampling EEG Spectrogram to the same resolution than fNIRS
 Ntseg=size(S,1);Nf=size(S,2);Nc=size(S,3);
 %Fsnew = FsN; %19.5312; %125; % Hz
 [~,indL] = min(mod(FsN*(1:2000),1)); % look for the closest integer multiplo
 Sc = reshape(S,Ntseg,Nf*Nc);
 Sc = resample_old(Sc,round(FsN*indL),round(Fs*indL));
 Sc(Sc(:)<=0)=eps*min(abs(Sc(:)));  % resampling can lead to non-positive values.
 %tEEGsc = (0 : size(Sc,1)-1)./FsN;
 %figure;imagesc(tEEGsc,freqVec,Sc(:,1:Nf)');axis xy
 Sc = reshape(Sc,size(Sc,1),Nf,Nc);
 tEEGdsc = t(1)+(0:size(Sc,1)-1)./FsN;
 %tEEGdsc = ( 1/Fsnew:1/Fsnew:size(Sc,1)*1/Fsnew) - (t(end)-t(1));% divided by round(Fsnew*indL)/indL??
 
 if 0	%
     dNIRS = spar;
     disp(['Run parafac fNIRS on ', num2str(t(end)-t(1))  's CH=', num2str(size(dNIRS,2)), ' WV=', num2str(size(dNIRS,3))]);
     dEEG = Sc;
     disp(['Run parafac EEG on ', num2str(t(end)-t(1)), 's Freq=', num2str(size(dEEG,2)), ' CH=', num2str(size(dEEG,3))]);
    ...
    for Ncomp = 2:4
        ...
        %figure;plot(tEEGdsc,HRF_TimeComponent(1:size(dNIRS,1),:))
...
%         end
 ...
 elseif 1 %old test 
    ...
    Hconv = convmtx(H.X(:,1,1),size(Sc,1)); % get convolution matrix    
    %Hconv = convmtx(dModel,size(Sc,1)); % get convolution matrix
    Sconv = pagemtimes(Hconv,Sc); % efficient convolution for matrices
    % chopping the obvious option: first part
    Sconv = Sconv(1:size(Sc,1),:,:);
    % check for other chopping?
    % explore correlations
    %figure; plot(tEEGdsc,[Sconv(:,1,1)./max(Sconv(:,1,1)) spar(:,1,1)./max(spar(:,1,1)) spar(:,1,2)./max(spar(:,1,2))])
...
[Xfactors,Yfactors,Core,B,ypred,ssx,ssy] = npls(Sconv.^2,dNIRS,Ncomp);
...
figure; % This is for plotting 
subplot(2,3,1); plot(tEEGdsc,AEEG);xlabel('Time (s)');ylabel('EEG spectrogram * HRF'); title('EEG')%ts_eeg(indseg)
subplot(2,3,2); plot(freqVec,BEEG);xlabel('Frequency (Hz)');ylabel('Normalized'); title('EEG') %axis tight
subplot(2,3,3); plot(1:length(chLabels),CEEG); set(gca, 'XtickLabels',chLabels); xlabel('Frequency (Hz)');ylabel('Normalized'); title('EEG')
subplot(2,3,4); plot(tEEGdsc,Anirs);xlabel('Time (s)');ylabel('fNIRS'); title('fNIRS')%ts_eeg(indseg)
subplot(2,3,5); plot(Bnirs); axis tight; xlabel('Channels');ylabel('Normalized'); title('fNIRS')
subplot(2,3,6); bar(Cnirs); axis tight; xlabel('Wavelenght');ylabel('Normalized');set(gca, 'XtickLabels',{'HbO','HbR'});title('fNIRS')
% try
% for icomp = 1:Ncomp
