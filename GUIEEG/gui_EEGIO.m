function varargout = gui_EEGIO(varargin)
% GUI_EEGIO M-file for gui_EEGIO.fig
%      GUI_EEGIO, by itself, creates a new GUI_EEGIO or raises the existing
%      singleton*.
%
%      H = GUI_EEGIO returns the handle to a new GUI_EEGIO or the handle to
%      the existing singleton*.
%
%      GUI_EEGIO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_EEGIO.M with the given input arguments.
%
%      GUI_EEGIO('Property','Value',...) creates a new GUI_EEGIO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui_EEGIO_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property
%      application
%      stop.  All inputs are passed to gui_EEGIO_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @gui_EEGIO_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_EEGIO_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui_EEGIO is made visible.
function gui_EEGIO_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_EEGIO (see VARARGIN)

% Choose default command line output for gui_EEGIO
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes gui_EEGIO wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = gui_EEGIO_OutputFcn(hObject, eventdata, handles) 
...
plot(PMI{currentsub}.data(cf).EEG.time,  PMI{currentsub}.data(cf).EEG.data);
...
% --- Executes on button press in btn_DATASCROLL.
function btn_DATASCROLL_Callback(hObject, eventdata, handles)
% hObject    handle to btn_DATASCROLL (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
PMI = get(guiHOMER,'UserData');
cf = PMI{currentsub}.currentFile;
if get(handles.popupmenu_montage,'value')==1
    var{1,1} = 'srate';
...
%     events(2).urevent = 1;
    ...
    var{1,12}= A;
    var{1,13}='winlength';
    var{1,14}=size(PMI{currentsub}.data(cf).EEG.data,1)*1/500; %time lenght

   % PMI{currentsub}.data(cf).EEG.data = [zeros(2000,20);PMI{currentsub}.data(cf).EEG.data; zeros(2000,20)]
eegplot(-single(PMI{currentsub}.data(cf).EEG.data(:,1:19)'),var{1},var{2},var{3},var{4},var{5},var{6},var{7},var{8},var{9},var{10},var{11},var{12},var{13},var{14});
t = get(gca,'xtick')

%set(gca,'xticklabels',num2str(PMI{currentsub}.data(cf).EEG.time(1)))

elseif get(handles.popupmenu_montage,'value')==2
1
...
% hObject    handle to popupmenu_montage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popupmenu_montage contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_montage
