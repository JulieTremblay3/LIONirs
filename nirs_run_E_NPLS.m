function out = nirs_run_E_NPLS(job)
%Use EEG 
% 
    T = readtable(job.e_NPLSxlsfiles{1});
    pathout = job.e_NPLSpathout;
    if job.m_NPLSmode ==0 %Run parafac for NIRS and EEG find the tempo
    
    for filenb=1:size(T,1)
        NIRS = [];
        NIRSmat =T.Subject{filenb};
        disp(['Apply decomposition on : ', T.Subject{1}]) ;
        load(NIRSmat);
        tstart = T.Tstart(filenb);
        tstop = T.Tstop(filenb);
        ifile = T.fileID(filenb);
        ...
        NkEEG = 4;
        NkNIRS= 4;
        AllLi = zeros(NC, size(rDtp,1)); %Channel x Bloc  
        fprintf('%s\n','File processed');
         
        d1 = fopen_NIR(rDtp{ifile,1},NC);
        d1 = d1'; %time x ch
        [dir1,fil1,~] = fileparts(rDtp{ifile});
        tHRF = 1/NIRS.Cf.dev.fs:1/NIRS.Cf.dev.fs:size(d1,1)*1/NIRS.Cf.dev.fs;
        listgood = find( NIRS.Cf.H.C.ok);

        idstart = find(tHRF<tstart);
        idstart = idstart(end);
        idstop = find(tHRF<tstop);
        idstop  = idstop(end);
        indt = idstart:idstop;
        intensnorm = d1(indt,listgood);
        t = tHRF(indt);
        ...
        spar = intensnorm - A;
        disp(['Load fNIRS: ',rDtp{ifile,1}, 'Interval: ' sprintf('% 5.2f',tHRF(indt(1))) ,' to ',sprintf('% 5.2f',tHRF(indt(end)))])
        disp('Detrend and select CH ')
        %reorganized in wavelenght HBO HBR
        spar = cat(3,spar(:,1:end/2),spar(:,end/2+1:end)); %CH x Time x WAVELENGHT    
        listgood = find( NIRS.Cf.H.C.ok(1:end/2)); %only first half for PARAFAC 
        ...
        for i=1:nbcomp
            ...
            tstartEEG = tHRF(idstart)+offset;
            tstopEEG =  tHRF(idstop)+offset;
            ...
            disp(['Load EEG : ' , filelist, ' Interval: ' sprintf('% 5.2f',tstartEEG), ' to ', sprintf('% 5.2f',tstopEEG)])
    
             %add_mirror_padding to avoid edge effect and cut the 
            if 1 
                dataseg = [fliplr(dataseg);dataseg;fliplr(dataseg)];
            end
            %Select good EEG channel for decomposition 
            infilevhdr = fullfile(dirNPLS,[file '.vhdr']);
            infovhdr = read_vhdr_brainvision(infilevhdr);

            if ~isempty(job.e_NPLSEEGELEfiles{1})
                [nameele,x,y,z] = readelefile(job.e_NPLSEEGELEfiles{1});
                channeluse = [];                
                  %selec good channel in eeg
                listgoodEEG =  [];            
                for ilabel=1:numel(nameele)
                    for ifind = 1:numel(infovhdr.label)
                        if strcmp(infovhdr.label{ifind }, nameele{ilabel});
                            listgoodEEG = [listgoodEEG,ifind ];  
                        end
                    end   
                end    
                dataseg = dataseg(:,listgoodEEG);
                chLabels = info.name_ele(1,listgoodEEG);             
            else
                disp('No subset of electrode open, use all EEG channel');
                chLabels = info.name_ele;
            end
            ...
            width = 5 ;
            %figure;plot(dataseg(:,:))
            disp('Compute wavelet transform 1 to 40 Hz')
     
            tic; TFRseg = morletcwtEd(dataseg,freqVec,Fs,width); toc % Complex Wavelets Coefficients
            ...
            tEEG = 1/Fs:1/Fs:size(dataseg,1)*1/Fs;
             idstartwithoutborder =   sum(tEEG< (tstopEEG- tstartEEG));
        

% figure;
% electrode = 1:numel(listgoodEEG)
% subplot(4,1,1)
% plot(tEEG(idstartwithoutborder:idstartwithoutborder*2)+tHRF(tstart)-(tstopEEG- tstartEEG),mean(mean(S(idstartwithoutborder:idstartwithoutborder*2,1:3,electrode),2),3))
% axis xy
% xlabel('Time (s) nirs timeline')
% ylabel('Delta')
% title( chLabels)
% subplot(4,1,2)
% plot(tEEG(idstartwithoutborder:idstartwithoutborder*2)+tHRF(tstart)-(tstopEEG- tstartEEG),mean(mean(S(idstartwithoutborder:idstartwithoutborder*2,4:7,electrode),2),3))
% axis xy
% xlabel('Time (s) nirs timeline')
% ylabel('Theta')
% subplot(4,1,3)
% plot(tEEG(idstartwithoutborder:idstartwithoutborder*2)+tHRF(tstart)-(tstopEEG- tstartEEG),sum(sum(S(idstartwithoutborder:idstartwithoutborder*2,8:12,electrode),2),3))
% axis xy
% xlabel('Time (s) nirs timeline')
% ylabel('Alpha')
% subplot(4,1,4)
% plot(tEEG(idstartwithoutborder:idstartwithoutborder*2)+tHRF(tstart)-(tstopEEG- tstartEEG),sum(sum(S(idstartwithoutborder:idstartwithoutborder*2,13:40,electrode),2),3))
% axis xy
% ylabel('Beta')
% xlabel('Time (s) nirs timeline')


        %LOAD HRF in auxiliairy file %SAME AS NIRS SAMPLE RATE AND TIME
        %FRAME
        filelist = NIRS.Dt.AUX.pp(end).p{ifile};   
        [dataAUX,infoAUX,labelAUX,ind_dur_chAUX] = fopen_EEG(filelist);
        FsAUX = 1/(infoAUX.SamplingInterval*1e-6);   
        daux = dataAUX;
        Label = infoAUX.name_ele{1};
        taux=1/FsAUX:1/FsAUX:(1/FsAUX*numel(daux));
        dModel = daux(idstart:idstop,1); 
        disp(['Load HRF : ' , filelist, ' Interval: ', sprintf('% 5.2f',taux(idstart)), ' to ', sprintf('% 5.2f',taux(idstop))])
   



 %% Downsampling EEG Spectrogram to fNIRS frequency 
 ...
 tEEGdsc = ( 1/Fsnew:1/Fsnew:size(Sc,1)*1/Fsnew) - (t(end)-t(1));%  avec padding avant et arriere temps 0 maintenant synchrone fNIRS. 

   
     dNIRS = spar; %need to be time x ch x wavelenght
    ...
    for Ncomp = 2:4
        ...
        %Correlation juste sur le temps commun de la nirs
...
%         end
    ...
    A = [Bchannel(chplot,icompNIRSmax) * CNIRS(1,icompNIRSmax)] ; 
    timecourse = Anirs(:,icompNIRSmax);
    disp(['Save: ', fullfile(job.e_NPLSpathout, ['TOPOHbO', num2str(t(1)),'to', num2str(t(end)),'_corrHRF', sprintf('%0.2f',rdNIRS_MODEL(  NcompNIRSmax,icompNIRSmax)),'.mat'])])
    save(fullfile(job.e_NPLSpathout, ['TOPOHbO', num2str(t(1)),'to', num2str(t(end)),'_corrHRF', sprintf('%0.2f',rdNIRS_MODEL(  NcompNIRSmax,icompNIRSmax)),'.mat']),'A')
    A = Bchannel(chplot,icompNIRSmax) * CNIRS(2,icompNIRSmax);
    save(fullfile(job.e_NPLSpathout, ['TOPOHbR', num2str(t(1)),'to', num2str(t(end)),'_corrHRF', sprintf('%0.2f',rdNIRS_MODEL(  NcompNIRSmax,icompNIRSmax)),'.mat']),'A')
    disp(['Save: ', fullfile(job.e_NPLSpathout, ['TOPOHbR', num2str(t(1)),'to', num2str(t(end)),'_corrHRF', sprintf('%0.2f',rdNIRS_MODEL(  NcompNIRSmax,icompNIRSmax)),'.mat'])])
   ...
   title(fileNIRS);
    ...
    A(listgood)=Bnirs(:,icompNIRSmax);
      ...
      fileout = fullfile(job.e_NPLSpathout,['CompNIRS_Time',...
        sprintf('%2.1f',tHRF(idstart)),...
        'to',sprintf('%2.1f',tHRF(idstop)),...
    ...
    for Ncomp  = 2:4
         ...
         [rEEGHRF_NIRS(Ncomp,1:Ncomp),pvalEEGHRF_NIRS(Ncomp,1:Ncomp)]  =  corr(HRF_TimeComponent(id:idstop,:),HRFNIRS_Corrwith_MODEL );
             figure; plot(HRF_TimeComponent(id:idstop,:));
    ...
    %hold on;plot(dModel)
    title('SECOND BEST CORRELATION NIRS EEG')
    HRF_TimeComponent = conv2(H.X(:,1,1)/10,AEEG);
    ...
    elseif 0
            ...
            S =  abs(TFRseg);
            tEEG = 1/Fsnew:1/Fsnew:size(dataseg,1)*1/Fsnew;
            chLabels = info.name_ele;
            for idisplay=1 %figure spectrum
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            clear TFRseg datasegR
            % plot ****************************
            indpt = 1:size(S,1);
            indpf = 1:size(S,2);
            indpc = 1:size(S,3);
            Measure = log(S(indpt,indpf,indpc)); tvec = tEEG(indpt); fvec = freqVec(indpf); %log(S(indpt,indpf,indpc)); %S(indpt,indpf,indpc);%
            Ncc = size(Measure,3); nrow = ceil(sqrt(Ncc)); ncol = ceil(Ncc/nrow);;
            cmax = max(Measure(:));cmin = min(Measure(:));
            if 0
            figure; 
            for c = 1:Ncc
                subplot(nrow,ncol,c); imagesc(tvec,fvec,Measure(:,:,c)'); 
                axis xy; caxis([cmin cmax]); title(chLabels{indpc(c)}); 
            end
                set(gcf,'name','EEG spectrum before convolution')
                end
            end
            ...
            delay = -2.3; % additional delay assumed
            Nthrf = round(Th*FsN); frametimes = (0:(Nthrf-1))./FsN;% time points for the Th seconds with the same resolution as nirs.
            %Nthrf = round(Th*Fs); frametimes = (0:(Nthrf-1))./Fs;% time points for the Th seconds with the same resolution as EEG.
            H = fmridesign(frametimes,0,[1 0],[],[5.4+delay 5.2 10.8+delay 7.35 0.35 0]);
            H = squeeze(H(:,1,1));
            %figure; plot(frametimes,H.X(:,1,1)); xlabel('Time (s)')
            ...
            Ntds = size(Sc,1);
            tEEGdsc =  1/Fsnew:1/Fsnew:size(Sc,1)*1/Fsnew;% divided by round(Fsnew*indL)/indL??
    ...
    Sc1 = conv2(H.X(:,1,1),Sc);
    Sc1 = reshape(Sc1,size(Sc1,1),Nf,Nc);
    Sc = reshape(Sc,size(Sc,1),Nf,Nc);
    ...
    %PARAFAC EEG WITHOUT CONVOLUTION 
    Nk = NkEEG;  % Number of Components to extract.
    opt = zeros(1,6); opt(3)=0;  figure;% only plotting
    const = [0 2 0]; % constraints, 1 for orthogonality, 2 for nonnegativity, 3 for unimodality
    %figure; % This is for plotting concordia
    [FactorsEEG,it,err,corcondia] = parafac(Sc,Nk,opt,const);
    
    %plotting 
    [A,B,C]=fac2let(FactorsEEG);
    ll = cellstr([repmat('Comp ',Nk,1) num2str((1:Nk)')]); % for the legend in figures
    %figure; 
    %subplot(2,2,1); plot(tEEGdsc,A);xlabel('Time (s)');ylabel('Data Units'); %ts_eeg(indseg)
    %subplot(2,2,2); plot(freqVec, B); axis tight; xlabel('Frequence (Hz)');ylabel('Normalized');
    %subplot(2,2,3); plot(C); set(gca,'Xtick',1:Ncc,'XTickLabel',chLabels); xlabel('Channels');ylabel('Normalized');legend(ll)
    %plotting 
    %disp(['Corcondia: ' num2str(corcondia) ' %'])
    %ll = cellstr([repmat('Comp ',Nk,1) num2str((1:Nk)')]); % for the legend in figures
    %set(gcf,'name','PARAFAC EEG without convolution')
    
    
    
    HRF_TimeComponent = conv2(H.X(:,1,1)/10,A);
    tHRFc = 1/Fsnew:1/Fsnew:size(H.X(:,1,1),1)*1/Fsnew;
    tconv = 1/Fsnew:1/Fsnew:size(Sc1,1)*1/Fsnew;
    id_cut_conv = numel(tconv) - numel(H.X(:,1,1))
    % figure;hold on
    % plot(tEEGdsc,A','displayname','Component EEG without convolution hrf')
    % plot(tconv(1:id_cut_conv), HRF_TimeComponent(1:id_cut_conv,:),'displayname','Component EEG with convolution hrf')
    % plot(tconv(1:id_cut_conv), dataAUX,'r', 'displayname','modelHRF')
    % for i=1:size(HRF_TimeComponent,2)
    %     corr_withHRFmodel(i) = corr(dataAUX,HRF_TimeComponent(1:id_cut_conv,i))
    % end
    % figure; 
    % for ik =1:Nk
    %     subplot(2,2,1);hold on; plot(tconv(1:id_cut_conv),HRF_TimeComponent(1:id_cut_conv,ik));xlabel('Time (s)');ylabel('Data Units'); %ts_eeg(indseg)
    %     subplot(2,2,2);hold on; plot(freqVec(:),B(:,ik),'displayname',['Corr with modelHRF',num2str(corr_withHRFmodel(ik))]); xlabel('Frequency (Hz)');ylabel('Normalized')
    % end
    % legend
    
    for i=0%CHECK EEG CONV with HRF
    
    figure;hold on
    plot(dataAUX)
    plot(tconv(1:id_cut_conv), HRF_TimeComponent(1:id_cut_conv,:))
    Measure = log(Sc(:,:,:)); tvec = tdsc(:); fvec = freqVec(:); %log(S(indpt,indpf,indpc)); %S(indpt,indpf,indpc);%
    Ncc = size(Measure,3); nrow = ceil(sqrt(Ncc)); ncol = ceil(Ncc/nrow);;
    cmax = max(Measure(:));cmin = 0 %min(Measure(:));
    figure; 
    for c = 1:Ncc
        subplot(nrow,ncol,c); imagesc(tvec,fvec,Measure(:,:,c)'); 
        axis xy; caxis([cmin cmax]); title(chLabels{indpc(c)}); 
    end
    set(gcf,'name','EEG spectrum after convolution')
    
    indnan = find(~isnan(Sc(:,1,1)));
    tpar = tdsc(indnan);
    % Options for PARAFAC
    indpf = 5:size(S,2);
    Tmd =  Sc1(indnan,indpf ,:);  %%Sc1 EEG convol,  Sc EEG without convol
    
    Nk = 3;  % Number of Components to extract.
    opt = zeros(1,6); opt(3)=0;  figure;% only plotting
    const = [0 2 0]; % constraints, 1 for orthogonality, 2 for nonnegativity, 3 for unimodality
    %figure; % This is for plotting concordia
    [Factors,it,err,corcondia] = parafac(Tmd,Nk,opt,const);
    ...
    ll = cellstr([repmat('Comp ',Nk,1) num2str((1:Nk)')]); % for the legend in figures
    figure; 
    subplot(2,2,1); plot(tpar,A);xlabel('Time (s)');ylabel('Data Units'); %ts_eeg(indseg)
    subplot(2,2,2); plot(freqVec(indpf ),B); xlabel('Frequency (Hz)');ylabel('Normalized')
    subplot(2,2,3); plot(C); set(gca,'Xtick',1:Ncc,'XTickLabel',chLabels); xlabel('Channels');ylabel('Normalized');legend(ll)
    %plotting 
    disp(['Corcondia: ' num2str(corcondia) ' %'])
    ll = cellstr([repmat('Comp ',Nk,1) num2str((1:Nk)')]); % for the legend in figures
    [Aeeg,Beeg,Ceeg]=fac2let(Factors);
    %*****************************************
    set(gcf,'name','PARAFAC EEG with HRF convolution')
    end
    
      %% do PARAFAC only on NIRS  ************************
    dNIRS = permute(d1,[2,1]); 
    dNIRS = reshape(dNIRS, size(dNIRS,1), size(dNIRS,2)/2,2)
    [ntt,Nch,ncc] = size(dNIRS);
    for ich = 1:Nch
        NchLabels(ich) = {num2str(ich)};
    end
    % Options for PARAFAC
    Nk = NkNIRS;  % Number of Components to extract.
    opt = zeros(1,6); opt(3)=0;  % only plotting
    const = [0 0 0]; % constraints, 1 for orthogonality, 2 for nonnegativity, 3 for unimodality
    
    [FactorsNIRS,it,err,corcondia] = parafac(dNIRS,Nk,opt,const);
    [ANEW,BNEW,C]=fac2let(FactorsNIRS);
    %plotting 
    ll = cellstr([repmat('Comp ',Nk,1) num2str((1:Nk)')]); % for the legend in figures
    
    for i=1:size(ANEW,2)
        corrNIRS_withHRFmodel(i) = corr(dataAUX,ANEW(1:id_cut_conv,i))
    end
    % figure; % This is for plotting concordia
    % subplot(2,2,1); plot(tHRF,ANEW);xlabel('Time (s)');ylabel('Data Units'); legend(ll)
    % subplot(2,2,2); plot(BNEW); axis tight; xlabel('Channels');ylabel('Normalized');legend(ll)
    % subplot(2,2,3); bar(C); set(gca,'XTickLabel',{'HbO','HbR'});ylabel('Normalized'); legend(ll)
    
    Anirs = ANEW
    
    Aeeg=Aeeg(1:size(Anirs,1),:)
    
    CorrT_par = corrcoef([Aeeg Anirs]); %CorrT_par = CorrT_par(Nk+1:end,1:3);
    [AEEG,BEEG,CEEG]=fac2let(FactorsEEG);
    
    corrcoef([Anirs(:,:),HRF_TimeComponent(1:id_cut_conv,:)])
    
    
    rNIRS_HRFEEG=abs(corrcoef([Anirs(:,:),HRF_TimeComponent(1:id_cut_conv,:)]))
    xCorrNIRS_HRFEEG = rNIRS_HRFEEG((size(Anirs,2)+1: size(Anirs,2)+size(Anirs,2)),1:size(HRF_TimeComponent,2))
    [Mc,Ic] = max(xCorrNIRS_HRFEEG)
    [Mr,Ir] = max(Mc);
    Ieeg = Ic(Ir);
    Inirs = (Ir);
    ...
    subplot(2,2,1)
    plot(Anirs(:,Inirs))
    title(['NIRS ', num2str(corr(HRF_TimeComponent(1:id_cut_conv,Ieeg ),Anirs(:,Inirs)))])
    subplot(2,2,3)
    plot(HRF_TimeComponent(1:id_cut_conv,Inirs ))
    title(['EEG ', num2str(corr(HRF_TimeComponent(1:id_cut_conv,Ieeg ),Anirs(:,Inirs)))])
    subplot(2,2,4)
    plot(freqVec , BEEG(:,Ieeg))
    title(['EEG freq'])
    set(gcf,'name', ['First Corr at ' num2str(corr(HRF_TimeComponent(1:id_cut_conv,Ieeg ),Anirs(:,Inirs)))])
    
    xCorrNIRS_HRFEEG(Ieeg,Inirs)=0;
    [Mc,Ic] = max(xCorrNIRS_HRFEEG)
    [Mr,Ir] = max(Mc);
    Ieeg = Ic(Ir);
    Inirs = (Ir);
    ...
    %Sc EEG sans conv
    %Sc1 EEG avec conv
    [Xfactors,Yfactors,Core,B,ypred,ssx,ssy] = npls(Sc1(1:id_cut_conv,:,:),dNIRS,Ncomp);
    ...
    figure; % This is for plotting concordia
    subplot(3,3,1); plot(tHRF,AEEG);xlabel('Time (s)');ylabel('Data Units'); %ts_eeg(indseg)
    subplot(3,3,2); plot(freqVec, BEEG); axis tight; xlabel('Frequence (Hz)');ylabel('Normalized');
    subplot(3,3,3); plot(CEEG); set(gca,'Xtick',1:Ncc,'XTickLabel',chLabels); xlabel('Channels');ylabel('Normalized');legend(ll)
    [Anirs,Bnirs,Cnirs]=fac2let(Yfactors);
    subplot(3,3,4); plot(tHRF,Anirs);xlabel('Time (s)');ylabel('Data Units'); %ts_eeg(indseg)
    subplot(3,3,5); plot( Bnirs); axis tight; xlabel('Frequence (Hz)');ylabel('Normalized');
    subplot(3,3,6); plot(Cnirs); set(gca,'Xtick',1:Ncc,'XTickLabel',chLabels); xlabel('Channels');ylabel('Normalized');legend(ll)
    
    rNIRS_HRFEEG=abs(corrcoef([Anirs,AEEG]))
    xCorrNIRS_HRFEEG = rNIRS_HRFEEG((size(Anirs,2)+1: size(Anirs,2)+size(Anirs,2)),1:size(AEEG,2))
    [Mc,Ic] = max(xCorrNIRS_HRFEEG)
    [Mr,Ir] = max(Mc);
    Ieeg = Ic(Ir);
    Inirs = (Ir);
    subplot(3,3,7)
    plot(Anirs(:,Inirs));hold on
    plot(dataAUX)
    title(['NIRS ', num2str(corr(dataAUX(: ),Anirs(:,Inirs))),'NIRS corr with HRF model'])
    subplot(3,3,8)
    plot(AEEG(1:id_cut_conv,Ieeg ))
    title(['EEG ', num2str(corr(AEEG(:,Ieeg ),Anirs(:,Inirs))),'EEG corr with nirs  and ', num2str(corr(dataAUX(: ),AEEG(:,Ieeg ))),'EEG corr with model'])
    subplot(3,3,9)
    plot(freqVec , BEEG(:,Ieeg))
    title(['EEG freq'])
    set(gcf,'name', ['First Corr EEGNIRS ' num2str(corr(AEEG(:,Ieeg ),Anirs(:,Inirs)))])
    A = zeros(numel(NIRS.Cf.H.C.ok)/2,1);
    A(find(NIRS.Cf.H.C.ok(1:end/2)))=Bnirs(:,Inirs);
    save(fullfile(dir1,['Comp.mat']),'A' );
    disp(['Save: ',fullfile(dir1,['Comp.mat']) ])
    
             %figure;plot(eventV)
            filename = ['NPLS EEGNIRS r', sprintf('%0.2f',(corr(AEEG(:,Ieeg ),Anirs(:,Inirs))))];
            HRFcurve.data = Anirs(:,Inirs);
            HRFcurve.ind_dur_ch=EEG.ind_dur_ch;label,ind_dur_ch
            HRFcurve.marker=EEG.marker;
            HRFcurve.infoBV = info;
    ...
    % figure;hold on
    % subplot(3,1,1)
    % plot(tHRF,Aeeg(:,1),'displayname','EEG')
    % title('EEG')
    % subplot(3,1,2)
    % plot(tHRF,Anirs(:,:),'displayname','NIRS')
    % title('NIRS')
    % subplot(3,1,3)
    % plot(tHRF,HRF_TimeComponent(1:id_cut_conv,:))
    % title('EEG component after conv')
