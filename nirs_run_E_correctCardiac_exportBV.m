function out = nirs_run_E_correctCardiac_exportBV(job)
%Remode linear trend
%filename prefix 
...
%     try
        ...
        NC = NIRS.Cf.H.C.N;
        fs = NIRS.Cf.dev.fs;
        fprintf('%s\n',['File apply target PCA for cardiac artifact']);
...
%             try
                ...
                channelonlyfirsthalf = 1:size(d,1)/2; %just for BV return to EEG data
                
                [dir1,fil1,ext1] = fileparts(rDtp{f});
                infilevmrk = fullfile(dir1,[fil1 '.vmrk']);           
                infilevhdr = fullfile(dir1,[fil1 '.vhdr']); 
                
                dir2 = job.E_correctCardiac_exportBVFolder{1};               
                if ~exist(dir2,'dir'), mkdir(dir2); end
                outfile = fullfile(dir2,[prefix fil1 '.dat']);
                outfilevmrk = fullfile(dir2,[prefix fil1 '.vmrk']);
                outfilevhdr = fullfile(dir2,[prefix fil1 '.vhdr']);

                d1 = d(channelonlyfirsthalf,:);            
                ...
                try
                    ...
                    try
                        ...
                        ChannelLabels = info.label(channelonlyfirsthalf);
                    catch
                        ChannelLabels = ConvertmlIDsrs2label(NIRS);
                    end
                    
                    x = NIRS.Cf.H.S.r.o.mm.p(1,:); 
                    y = NIRS.Cf.H.S.r.o.mm.p(2,:) ;
                    z = NIRS.Cf.H.S.r.o.mm.p(3,:) ;
                    [coor_theta, coor_phi, coor_r] = cart2sph(y,x,z);
                    infoBV.coor_theta = coor_theta*180/pi;
                    infoBV.coor_phi =  coor_phi*180/pi;
                    infoBV.coor_r = coor_r;
                    ...
                    nirs_boxy_write_vhdr(outfilevhdr,... %Output file
                        ...
                        SamplingInterval,...
                        size(d,2),infoBV); %SamplingInterval in microseconds
                catch
                end
                 if DelPreviousData
                    delete(rDtp{f,1});
...
%                 if f == 1
%                     NIRS.Dt.fir.pp(lst+1).pre = 'PCA onset artefac';
