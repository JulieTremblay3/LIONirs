        ...
        if isfield(job,'c_SegmentAUDIO')
                ...
                if isfield(job.c_SegmentAUDIO, 'b_SegmentAUDIOYes')
                    ...
                    try
                        ...
                        id = strfind(MException.message,'TotalSamples is');
                        if ~isempty(id)
                            idstopvideo = str2num(MException.message(id+15:end-1));
                            disp( ['Warning video ',NIRS.Dt.Video.pp(end).p{f},sprintf(' %4.1f',idstopvideo*1/infovideo.SampleRate/60), ' min ' ,'is shorter than fNIRS ', sprintf('%4.1f',idstop*1/infovideo.SampleRate/60), 'Min'])
                            [yaudio,Fsaudio] = audioread(NIRS.Dt.Video.pp(end).p{f}, [idstart idstopvideo]);
                        else
                            disp(['Error reading video.', NIRS.Dt.Video.pp(end).p{f}])
                        end
                    end
                    toc
                    disp(['Write audio...'])
                    ...
                    if job.c_SegmentAUDIO.b_SegmentAUDIOYes.m_SegmentAUDIOformat==0 %mp3
                        [filepath,name,ext]=fileparts(pathaudioout);

                        fileoutaudio = fullfile(filepath, [name,'.mp3']) ;
                    elseif job.c_SegmentAUDIO.b_SegmentAUDIOYes.m_SegmentAUDIOformat==1 %wav
                        [filepath,name,ext]=fileparts(pathaudioout)
                        fileoutaudio = fullfile(pathaudioout, [name,'.wav']) ;
                    end
                    try
                        audiowrite(fileoutaudio, yaudio,Fsaudio);
                    catch
                        [file,location] = uiputfile
                        audiowrite(fullfile(loation,file), yaudio,Fsaudio);
                    end
                    toc
