...
%Ajout de la matrice de transformation modifier en entrée
function [Helm_MRI_Coords, Helm_MRI_Norms ] = CoRegisterHelmetToMRI_TF( oHelmet, oMRI_Data,projection,mat_TF )
	...
	Helm_MRI_Coords = matCoordsH*inv(mat_TF);
%     Create_6DOF_CoRegistration_Matrix( matHelmetRefs,                matMRIRefs, ...
...
%                                                                     IsHelmetRH,                   IsMRIRH );
    ...
    %(anisotropiques??).
    Helm_MRI_Norms = matNormalsH*inv(mat_TF);%Create_3DOF_CoRegistration_Matrix( matHelmetRefs,                matMRIRefs, ...
%                                                                     IsHelmetRH,                   IsMRIRH );
