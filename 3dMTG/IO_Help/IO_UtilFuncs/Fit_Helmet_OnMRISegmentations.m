...
%
%
function oHelmet = Fit_Helmet_OnMRISegmentations( oHelmet, oMRI_Data, bProcessSkin, bProcessCortex, bDisplay )
        ...
        %----------------------------------------------------------------------------------------------------------
         if( bProcessSkin && ~isempty(matSkinSegmentation) ) %IN VOXEL JT 
            ...
            if pElem == size(HCoords,1)
                skindepth = vHoles(pElem).SkinDepth;
                nx = vHoles(pElem).Normal.x;
                ny = vHoles(pElem).Normal.y;
                nz = vHoles(pElem).Normal.z;
            end
        end
