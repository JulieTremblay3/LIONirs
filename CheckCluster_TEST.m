...
%     
      load('C:\data\Data_NIRS\BebeResting\WAVMAT\CLU_C07_001_HBO.mat')
       X = reshape(TFR, size(TFR,1)*size(TFR,2),size(TFR,3));
       idx = find(C{8}.IDX == 2);
        figure;imagesc(reshape(C{8}.IDX == 2,size(TFR,1),size(TFR,2)))
     for i=1:size(TFR,3)
     in1 = X(idx,i);
    ...
    C = clusterMATCORR(abs(TFR),Args.nbcluster)
