        ...
        for iAUX=1:numel(NIRS.Dt.AUX)
            for imodule = 1:numel(NIRS.Dt.AUX(iAUX).pp)
                for ifile = 1:numel(NIRS.Dt.AUX(iAUX).pp(imodule).p)
                    ...
                    if ~isempty(fileloc_previous)
                        [dir1,file1,ext1]=fileparts(fileloc_previous);
                    if strcmpi(dir1(end-5:end),dirrootnew(end-5:end))
                        NIRS.Dt.AUX(iAUX).pp(imodule).p{ifile} = fullfile( dirrootnew , [file1,ext1]);
                    else
                        tmpdir=split(dir1,filesep);
                        tmpdir1=fullfile(tmpdir{1:end-1});
                        if strcmpi(tmpdir1(end-5:end),dirrootnew(end-5:end))
                            NIRS.Dt.AUX(iAUX).pp(imodule).p{ifile} = fullfile( dirrootnew ,tmpdir{end}, [file1,ext1]);
                        end
                    end
                    end
                end
